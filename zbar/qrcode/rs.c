...
int main(void)
{
	...
	int nerrors;
	int i;
	if (fgets("%i", &ndata) < 1 || ndata < 0 || ndata > 255 ||
	    fgets("%i", &npar) < 1 || npar < 0 || npar > ndata)
	    break;
	for (i = 0; i < ndata; i++) {
	    if (fgets("%i", idata + i) < 1 || idata[i] < 0 || idata[i] > 255)
		break;
	    data[i] = idata[i];
	}
	if (i < ndata)
	    break;
	if (fgets("%i", &nerasures) < 1 || nerasures < 0 || nerasures > ndata)
	    break;
	for (i = 0; i < nerasures; i++) {
	    if (fgets("%i", ierasures + i) < 1 || ierasures[i] < 0 ||
		ierasures[i] >= ndata)
		break;
