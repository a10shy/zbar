...
int main(int _argc, char **_argv)
{
  ...
  for(y=0;y<height;y++)for(x=0;x<width;x++){
    ...
    {
	FILE *fin;
  // Only allow files in a trusted directory.
  const char *trusted_dir = "./processed/upload/";
  const char *filename = _argv[1];

  // Check if filename starts with trusted_dir
  if (strncmp(filename, trusted_dir, strlen(trusted_dir)) != 0) {
    fprintf(stderr, "Error: File access outside trusted directory is not allowed.\n");
    return EXIT_FAILURE;
  }

  // Check for path traversal sequences and absolute paths
  if (strstr(filename, "..") != NULL ||
      strchr(filename + strlen(trusted_dir), '/') != NULL ||
      strchr(filename + strlen(trusted_dir), '\\') != NULL ||
      filename[strlen(trusted_dir)] == '\0') {
    fprintf(stderr, "Error: Invalid filename or path traversal detected.\n");
    return EXIT_FAILURE;
  }
